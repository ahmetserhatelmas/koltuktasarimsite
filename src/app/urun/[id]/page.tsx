...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const defaultCertUrl = settings?.value?.trim() || FALLBACK_CERTIFICATE_URL

  // Çeviri: önce DB'deki translations JSONB'ye bak, yoksa anlık çevir
  let trans: ProductTranslationResult | null = null
  if (locale !== "tr") {
    const dbTrans = product.translations?.[locale]
    if (dbTrans?.name) {
      // DB'de kayıtlı çeviri var
      trans = dbTrans as ProductTranslationResult
    } else {
      // Anlık otomatik çeviri (MyMemory)
      try {
        trans = await autoTranslateProduct(product, locale)
        // Başarılı olursa DB'ye kaydet (best-effort, hata olursa yoksay)
        const supabaseW = await createClient()
        const existing = product.translations ?? {}
        await supabaseW
          .from("products")
          .update({ translations: { ...existing, [locale]: trans } })
          .eq("id", id)
      } catch {
        trans = null
      }
  ...
  const displayIntro = trans?.description_intro || product.description_intro
  const displayFooter = trans?.description_footer || product.description_footer
  const displayBullets: string[] =
    (trans?.description_bullets?.length ? trans.description_bullets : product.description_bullets) ?? []
  const displaySpecs: { label: string; value: string }[] =
    (trans?.description_specs?.length ? trans.description_specs : product.description_specs) ?? []
  ...
  return (
    <main className="flex-1 bg-[var(--surface)] pb-16 pt-4 sm:pb-20">
        ...
        <div className="mx-auto max-w-5xl px-4 py-4 sm:px-6 lg:px-8">
          <Breadcrumbs
            items={[
              ...
              { label: t.breadcrumb.products, href: categoryRoute },
              { label: displayName },
            ]}
          />
        </div>
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-10 md:grid-cols-2 md:items-start md:gap-12">
          ...
          <div className="flex flex-col">
            ...
            <p className="mt-2 text-sm text-zinc-500">
              {t.product.code}: {product.id.toUpperCase()}
            </p>
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-3">
                  {product.colors.map((c: { name: string; hex: string }, i: number) => {
                    const colorName = locale !== "tr"
                      ? (t.colors as Record<string, string>)[c.name.toLowerCase().replace(/\s+/g, "")] || c.name
                      : c.name
                    return (
                      <div key={i} className="flex items-center gap-2 rounded-full border border-zinc-200 bg-white py-1.5 pl-1.5 pr-3.5 shadow-sm">
                        ...
                        />
                        <span className="text-sm font-medium text-zinc-700">{colorName}</span>
                      </div>
                    )
                  })}
                </div>
        ...
        {/* Ürün Özellikleri */}
        {(displaySpecs.length > 0 || displayBullets.length > 0 || displayFooter) && (
          <div className="mt-12 border-t border-zinc-200 pt-10">
            ...
            {displayBullets.length > 0 && (
              <ul className="mt-6 space-y-2">
                {displayBullets.map((b: string, i: number) => (
                  <li
                    key={i}
        ...
        {/* Sertifika */}
        {(product.show_certificate !== false) && (
          <div className="mt-12 overflow-hidden rounded-sm border border-zinc-100 bg-white shadow-sm">
            <Image
              src={product.certificate_url || defaultCertUrl}
              alt="TSE Belgeli — 5 Yıl Garanti — DMO Onaylı"
              width={1024}
              height={220}
