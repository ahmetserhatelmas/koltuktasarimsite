...
export function ProductCard({
  ...
  const trans = locale !== "tr" ? (product.translations?.[locale] as { name?: string } | undefined) : undefined
    ...
    >
        ...
        {pct > 0 ? (
          <div className="absolute left-2 top-2 z-10 flex flex-col gap-1">
            <span className="rounded-sm bg-zinc-900 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
              {t.product.discount.replace("%{n}", String(pct))}
            </span>
          </div>
        ) : null}
        <Image
      ...
      <div className="flex flex-1 flex-col border-t border-zinc-100 px-3 pb-3 pt-3">
        <Link href={productDetailHref(product.id)} className="flex-1">
          <h3 className="line-clamp-2 text-sm font-semibold leading-snug text-zinc-900">{displayName}</h3>
          <p className="mt-1 text-xs text-zinc-400">{t.product.code}: {product.id.toUpperCase()}</p>
          <p className="mt-2 text-sm text-zinc-500">
            {quote ? (
              <span className="font-bold text-zinc-900">{t.product.price_quote}</span>
            ) : (
              <>
